...
class Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        adj_entity1 = np.zeros([max_n_node, config.sample_num], dtype=np.int64)
        adj_entity2 = np.zeros([max_n_node, config.sample_num], dtype=np.int64)
        for i in range(max_n_node):
            neighbor = list(copy.deepcopy(self.adj[items[i]]))
            n_neighbor = len(neighbor)
            if n_neighbor == 0:
                continue
            if n_neighbor >= config.sample_num:
                adj_entity1[i] = np.random.choice(neighbor, size=config.sample_num, replace=False)
            else:
                adj_entity1[i] = np.random.choice(neighbor, size=config.sample_num, replace=True)  
        ...
        for i in range(max_n_node):
            ...
            if n_neighbor == 0:
                continue
            if n_neighbor >= config.sample_num:
                adj_entity2[i] = np.random.choice(neighbor, size=config.sample_num, replace=False)
            else:
                adj_entity2[i] = np.random.choice(neighbor, size=config.sample_num, replace=True)  
                
        return [torch.tensor(alias_inputs), torch.tensor(adj), torch.tensor(items),
                torch.tensor(mask), torch.tensor(target), torch.tensor(u_input), torch.tensor(adj_entity1), torch.tensor(adj_entity2)]
