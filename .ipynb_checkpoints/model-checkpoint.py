...
class SessionGraph(nn.Module):
    ...
    def forward(self, inputs, adj, mask_item, item, first_adj1, first_adj2):
        ...
        item_neighbors2 = [inputs, first_adj2.view(batch_size, support_size)]

        for i in range(1, config.hop):
            item_sample_i = self.sample(item_neighbors2[-1])
            support_size *= neighbor_num[i]
            item_neighbors2.append(item_sample_i.view(batch_size, support_size))

        entity_vectors = [self.embedding(i) for i in item_neighbors2]
        ...
        h_global2 = entity_vectors[0].view(batch_size, seqs_len, config.dim)
        h_global2 = F.dropout(h_global2, config.dropout_global, training=self.training)


        
        neighbor_num = [config.sample_num, 2]
        support_size = seqs_len * neighbor_num[0]
        item_neighbors = [inputs, first_adj1.view(batch_size, support_size)]
        ...
        output = h_local + h_global + 0.1*h_global2
        return output
