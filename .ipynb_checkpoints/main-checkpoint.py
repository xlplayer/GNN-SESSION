...
def forward(model, data):
    alias_inputs, adj, items, mask, targets, inputs, first_adj1, first_adj2 = data
    alias_inputs = trans_to_cuda(alias_inputs).long()
    items = trans_to_cuda(items).long()
    adj = trans_to_cuda(adj).float()
    mask = trans_to_cuda(mask).long()
    inputs = trans_to_cuda(inputs).long()
    first_adj1 = trans_to_cuda(first_adj1).long()
    first_adj2 = trans_to_cuda(first_adj2).long()
    
    hidden = model(items, adj, mask, inputs, first_adj1, first_adj2)
