...
for k,v in tqdm(adj_all.items()):
    ...
    for i in range(len(v)):
        if num < sample_num: #or (seq2fre[v[i]] == seq2fre[v_t[sample_num-1]]):
            if not flag[v[i]]:
                v_t.append(v[i])
                num += 1
                
                if len(v[i]) == 3:
                    flag[tuple(sorted([v[i][0],v[i][1]]))] = 1
                    flag[tuple(sorted([v[i][0],v[i][2]]))] = 1
                    flag[tuple(sorted([v[i][1],v[i][2]]))] = 1
